...
# rescue節に例外クラスを指定した場合、捕捉されるのはそのクラス自身とそのサブクラスになります。
...
# 参考 :name と name: の違い
# :name はシンボルです。Rubyでは、シンボルは主に識別子として使われます。
# シンボル自体には値が含まれているわけではなく、名前（識別子）として扱われます。
# name: はハッシュのキーを示すための構文で、シンボル :name をキーとして使っているという意味です。
# 例
# person = { name: 'Alice' }
...
# メソッド内部でヒアドキュメントを使う場合など、識別子をインデントさせたい場合に便利です。
# 通常のヒアドキュメントでは識別子（TEXT）をインデントできませんが、<<- を使うと識別子をインデント可能です。
...
# <<~を使うと内部の文字列をインデントさせても先頭の空白部分が無視されます。
def some_method
    <<~TEXT
        こんにちは
        さようなら
...
# sprintfメソッドを使うと指定されたフォーマットの文字列を作成することができます。
# 以下は少数3位まで数字を表示する文字列を作成するコード例です。
sprintf('%0.3f', 1.2) #=>"1.200"

# ”フォーマット文字列 % 表示したいオブジェクト”の形式で書いても同じ結果が得られます。
'%0.3f' % 1.2 #=> "1.200"

# 表示したいオブジェクトを複数渡すこともできます
...
# 他のプログラミング言語では1文字だけの文字と、複数文字の文字列を別々に扱うものもありますが、
# Rubyでは違いはありません。いずれも文字列(Stringオブジェクト)として扱われます。
...
# 真偽値と条件分岐についてもっと詳しく
# &&や||を使った場合、式全体の戻り値は必ずしもtrueまたはfalseになるとは限りません。
# たとえば次の結果を見てください。

# Rubyは式全体が真または偽であることが決定するまで左から順に式を評価します。

# すべての式を評価する必要があったため、最後の式である3が戻り値になっています。
1 && 2 && 3 #=> 3
# 2つめのnilを評価した時点で式全体の真偽値が偽であることが確定したため、そこで評価を終了し、nilを返します。
1 && nil && 3 #=> nil
1 && false && 3 #=> false

nil || false #=> false
false || nil #=> nil
# 2を評価した時点で式全体の真偽値が真であることが確定したため、2を返します。
nil || false || 2 || 3 #=> 2

# 以下は||を使った式の戻り値や、真偽値が確定した時点で評価が終了されることを活用した架空のコード例です。
# Alice,Bob,Carolと順に検索し、最初にみつかったユーザ(nilまたはfalse以外の値を変数に格納する
...
# 条件演算子(三項演算子)
# Rubyでは? :を使った条件分岐(三項演算子)を使うことができます。
...
# 上のコードを三項演算子を使うと次のように書き直すことができます。
n > 10 ? '10より大きい' : '10以下'

# 次のように条件分岐した結果を変数に代入することも可能です。
...
# ?で終わるメソッドは慣習として真偽値を返すメソッドになっています。
...
# ?で終わるメソッドは自分で定義することもできます。
# 真偽値を返す目的のメソッドであれば、?で終わらせるようにしたほうが良いでしょう。
...
# Ruby3.0ではendを省略して1行でメソッドを定義できるエンドレスメソッド定義構文が導入されました。
# メソッド内の処理が1行で終わるような場合は、エンドレスメソッド定義構文を使うとより短く書ける場合があります。
...
# ただし、メソッド名と=の間にスペースがないと構文エラーになる
def greet= 'Hello'
#=> syntax error
...
# 擬似変数
# nilとtrueとfalseは擬似変数と呼ばれる特殊な変数です。
# このほかにもRubyには次のような擬似変数があります。
# self:オブジェクト自身
...
# 変数のを他の変数に代入したり、メソッドの引数として渡したりすると、新しい変数や
# メソッドの引数は元の変数と同じオブジェクトを参照します。
# 異なる変数が同じオブジェクトを参照しているかどうかはobject_idメソッドを使うとわかります。
...
# 同じオブジェクトと参照している場合、オブジェクトの状態が変更されると、
# その変更がそのまま各変数に影響します。
a = 'hello'
b = 'hello'
c = b
...
# 組み込みライブラリでない標準ライブラリやgemを利用する場合は、明示的にそのライブラリを読み込む必要があります。
# ライブラリを読み込む場合は次のような構文を使います。
# require ライブラリ名

# Dateクラスを使いたい場合はdateライブラリを読み込みます。
require 'date'
...
# ちなみに、requireを使って自作のRubyプログラムを読み込むこともできます。
# その場合は絶対パスもしくはRubyを実行しているカレントディレクトリからの相対パス(.で始まることが必須)で指定します。
# ただし、この方法は推奨されません。
# requireで自作Rubyプログラムを読み込むこともできるが非推奨
require './foo/bye'
# この方式で相対パスを指定すると実際に読み込まれるファイルがカレントディレクトリによって変わってくるためです。
# 適切なカレントディレクトリでRubyが実行されなかった場合、意図しない同名ファイルを読み込んでしまい、
# それがセキュリティ上の脆弱性に発展する可能性があります。
# 自作のRubyプログラムを読み込む場合はrequireではなく、require_relativeを使うようにしてください。




# putsメソッド、printメソッド、pメソッド、ppメソッド
# ターミナル(正確には標準出力)への出力でよく使われるこれらのメソッドについて。

# putsメソッドは改行を加えて変数の内容やメソッドの戻り値をターミナルに出力します。
# また、putsメソッドの戻り値はnilになります。
puts 123
123
=> nil
puts abc
...
# pメソッドは改行を加えて出力します。ただし文字列を出力すると
# ダブルクォートで囲まれている点がputsメソッドと異なります。
# また、pメソッドは引数で渡されたオブジェクトそのものがメソッドの戻り値になります。
# p 123
# 123
# => nil
# p 'abc'
...
# ppメソッドは複雑な配列やハッシュ、オブジェクトの内容を見やすく整形して出力します。
# pメソッドでは内容がごちゃごちゃして確認しづらいというときにppメソッドを使うと便利な場合があります。
...
# 改行文字を含む文字列をそれぞれのメソッドに渡してみましょう。

# putsメソッドやprintメソッドでは、文字列が"abc"と"def"の間で改行して出力されます。
# pメソッドでは改行文字(\n)が改行文字のまま出力されます。
s = "abc\ndef"
puts s
# abc
...
# では、この4つのメソッドの使い分けとしては、putsメソッドとprintメソッドは一般ユーザ向け、
# pメソッドとppメソッドは開発者向け、というふうに分かれます。
# putsメソッドとprintメソッドは内部的に引数で渡されたオブジェクトに対してto_sメソッドを呼び出して文字列に変換しています。
# 一方、pメソッドはto_sメソッドではなく、inspectメソッドを呼び出します。
# p 'abc'の出力結果がダブルクォートで囲まれて出力されたのは、内部的にStringクラスのinspectメソッドが呼び出されたためです。

# to_sメソッドもinspectメソッドもオブジェクトの内容を文字列に変換するメソッドですが、
# 一般的にinspectメソッドは開発者にとって役立つ情報を返すように実装されています。
# よって、pメソッドのほうが開発者向けだと見なすことができます。

# putsメソッドとprintメソッドは配列の表示形式では違いがあるものの、どちらも用途的には一般ユーザ向けという点では同じです。





# メソッドの呼び出しでカッコを付ける、付けないの使い分け

# 慣習的に次のようなケースはカッコが省略されることが多いです。
